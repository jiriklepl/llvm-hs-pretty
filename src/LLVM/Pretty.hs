...
ppGlobal g
  ...
  | GlobalAlias {} <- g = do
    let Right typ = getElementType $ LLVM.AST.Global.type' g
...
ppInstructionMetadata meta =
  fmap commas $ traverse (uncurry go) meta
  where
    go x = \case
      MDRef r -> return $ pretty r
      MDInline y -> do
        prettyY <- ppMDNode y
        return $ "!" <> short x <+> prettyY
...
ppInstruction i
  ...
  | ShuffleVector {..} <- i = do
      ...
      prettyMask <- ppTypedM ppShuffleMask mask
      prettyMeta <- ppInstrMeta metadata
...
ppDISubrange Subrange {..} = do
  ...
  return $ ppDINode "DISubrange" [("count", Just prettyCount)]
...
ppShuffleMask m = do
  ~(Right ta) <- typeOf m
  prettyM <- mapM (ppOperand . (ConstantOperand . C.Int 32 . fromIntegral)) m
  return $ pretty ta <+> ("<" <+> commas prettyM <+> ">")
